      ...
      <div className="w-full max-w-md rounded-t-2xl bg-white px-5 pt-5 pb-8 shadow-2xl sm:rounded-2xl">
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-full text-muted-gray hover:bg-stone-surface"
          >
            <X />
          </button>
        </div>
        ...
        <button
          ...
          disabled={saving}
          className="mb-4 flex w-full items-center justify-center gap-1.5 rounded-lg border border-stone-surface py-2 text-[13px] text-muted-gray hover:bg-cream-canvas disabled:opacity-50"
        >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-full bg-stone-surface text-[18px] font-bold text-heading-charcoal hover:bg-[var(--color-honey)]/20"
              >
                −
              </button>
              <input
                ...
                className="w-20 rounded-lg border border-stone-surface px-3 py-2 text-center text-[16px] font-semibold outline-none focus:border-[var(--color-link-blue)]"
              />
              <button
                type="button"
                onClick={() => setQuantity((q) => q + 1)}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-stone-surface text-[18px] font-bold text-heading-charcoal hover:bg-[var(--color-grass-green)]/20"
              >
                +
              </button>
            </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-stone-surface px-3 py-2 text-[14px] outline-none focus:border-[var(--color-link-blue)]"
            />
            {showSuggestions && filteredSuggestions.length > 0 && (
              <ul className="absolute z-10 mt-1 w-full rounded-lg border border-stone-surface bg-white shadow-subtle-3 max-h-36 overflow-y-auto">
                {filteredSuggestions.map((s) => (
                  <li key={s}>
                    <button
                      type="button"
                      className="w-full px-3 py-2 text-left text-[13px] hover:bg-cream-canvas"
                      onMouseDown={() => {
                        setSupplier(s);
          ...
          <div>
            ...
            <div className="flex items-center gap-2 rounded-lg border border-stone-surface px-3 py-2 focus-within:border-[var(--color-link-blue)]">
